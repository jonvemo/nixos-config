    ...
    for FORMAT in $INPUT_FORMATS
        for FILE in *.$FORMAT
            ...
            if contains -- $FORMAT $RESTRICTED_INPUTS; or begin
                ...
                if not contains -- $OUTPUT_FORMAT png # Añadida condición para omitir ffmpeg si OUTPUT_FORMAT es png
                    ...
                    rm -f "$INTERMEDIATE_FILE" # Eliminar intermedio solo si se realizó el paso 2
                else # Si OUTPUT_FORMAT es png, el archivo convertido es directamente el intermedio
                    set FILE_CONVERTED "$INTERMEDIATE_FILE" # Redefinir FILE_CONVERTED al intermedio
                    # No eliminar el intermedio, ahora es el archivo final
                end
